...
exports.createBusiness = async (req, res) => {
  try {
    const { businessName, address, sector, businessType, businessOwnerId } = req.body.trim();

    const businessOwner = await UserDataAccess.findUserByIdDB(businessOwnerId);

    if (!businessOwner) return Response.withError(res, BusinessError.businessOwnerCouldnotFound());
    const doesSectorExist = await SectorDataAccess.findSectorByIdDB(sector);
    if (!doesSectorExist) return Response.withError(res, BusinessError.sectorCouldnotFound());

    const doesBusinessTypeExist = await BusinessTypeDataAccess.findBusinessTypeByIdDB(businessType);
    if (!doesBusinessTypeExist) return Response.withError(res, BusinessError.businessTypeCouldnotFound());
    ...
    if (!businessOwner.roles.includes(Constants.ROLES.EMPLOYEE)) businessOwner.roles.push(Constants.ROLES.EMPLOYEE);
...
exports.updateBusiness = async (req, res) => {
  ...
  try {
    ...
    const doesBusinessTypeExist = await BusinessType.findById(updatedBusinessType);
    if (!doesBusinessTypeExist) return Response.withError(res, BusinessError.businessTypeCouldnotFound());
...
exports.profile = async (req, res) => {
  ...
  try {
    ...
    if (!business) return Response.withError(res, BusinessError.businessCouldnotFound());

    Response.success(res, 200, business, 'İş yeri profiline başarıyla ulaşıldı.');
  } catch (error) {
    if (error instanceof ValidationError) {
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
    ...
    if (error instanceof CastError) {
      error.message =
        'Görüntülenmek istenen iş yeri id değeri hatalı. ' + 'Lütfen iş yeri Id değerlerini doğru giriniz.';
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
    }
    console.log(error);
...
exports.addService = async (req, res) => {
  ...
  try {
    ...
    if (!foundService) return Response.withError(res, AdminError.serviceNotFound());

    const businessTypeId = mongoose.Types.ObjectId(foundBusiness.businessType);
    const serviceBusinessType = mongoose.Types.ObjectId(foundService.businessType);
    ...
    if (doesServiceExists) return Response.withError(res, BusinessError.ServiceAlreadyExist());
    ...
    if (error instanceof CastError) {
      error.message = 'Tanımlanmak istenen servis bulunamadı! ' + 'Lütfen Servis Id değerini doğru giriniz.';
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
    }
    Response.withError(res, CommonError.serverError());
...
exports.deleteService = async (req, res) => {
  ...
  try {
    ...
    const deletingService = business.serviceList.find(serviceObj => serviceObj._id.toString() === serviceId.toString());
    if (!deletingService) return Response.withError(res, AdminError.serviceNotFound());
    ...
    if (error instanceof CastError) {
      error.message = 'Servis silinemedi. Lütfen silinmek istenen servis/iş yeri' + ' Id değerlerini doğru giriniz!';
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
    }
    Response.withError(res, CommonError.serverError());
...
exports.hireEmployee = async (req, res) => {
  ...
  try {
    ...
    const isEmployed = business.employeeList.find(emp => emp._id.toString() === userId);

    if (isEmployed) return Response.withError(res, BusinessError.employeeAlreadyExist());

    if (!user.roles.includes(Constants.ROLES.EMPLOYEE)) user.roles.push(Constants.ROLES.EMPLOYEE);

    UserDataAccess.updateUserRolesDB(userId, user.roles);
    business.employeeList.push(user);
    business.save();
    Response.success(res, 200, { user, business }, 'Çalışan, iş yerine başarıyla tanımlandı.');
  } catch (error) {
    if (error instanceof ValidationError) {
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
...
exports.dischargeEmployee = async (req, res) => {
  ...
  try {
    ...
    if (!isEmployed) return Response.withError(res, BusinessError.employeeNotFound());
    ...
    Response.success(res, 200, { user, business }, 'Çalışan belirtilen iş yerinin çalışan listesinden çıkartıldı.');
  } catch (error) {
    if (error instanceof ValidationError) {
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
...
exports.assignService = async (req, res) => {
  ...
  try {
    ...
    if (!service) return Response.withError(res, BusinessError.serviceNotProvided());

    console.log('newPr => ', service);

    const foundEmployee = business.employeeList.find(emp => emp._id.toString() === employeeId.toString());

    console.log('foundEmployee => ', foundEmployee);

    // Servisin tanımlanmak istendiği çalışan bu iş yerinde çalışıyor mu?
    if (!foundEmployee) return Response.withError(res, BusinessError.employeeNotFound());
    ...
    const isAlreadyProvided = foundEmployee.providingServices.find(
      providingService => providingService._id.toString() === serviceId.toString()
    );

    if (isAlreadyProvided) return Response.withError(res, BusinessError.serviceAlreadyProviding());
    ...
    await BusinessDataAccess.updateProvidingServicesListDB(business._id, service);
    Response.success(res, 200, foundEmployee, 'Servis, çalışana başarıyla tanımlandı.');
  } catch (error) {
    if (error instanceof ValidationError) {
      Object.assign(error, { statusCode: 400 });
      console.log(error);
...
exports.removeService = async (req, res) => {
  ...
  try {
    ...
    const employee = business.employeeList.find(emp => emp._id.toString() === employeeId);

    if (!employee) return Response.withError(res, BusinessError.employeeNotFound());

    const removingService = employee.providingServices.find(service => service._id.toString() === serviceId.toString());
    if (!removingService) return Response.withError(res, BusinessError.serviceNotProvided());

    console.log('REMOVING SERVICE: ', removingService);
    employee.providingServices.remove(removingService);
    await business.save();
    Response.success(res, 200, business, 'Servis, tanımlanan çalışandan başarıyla silindi.');
  } catch (error) {
    if (error instanceof ValidationError) {
      Object.assign(error, { statusCode: 400 });
      return Response.withError(res, error);
